...
class SubscriptionMultiVersion(_PluginBase):
    ...
    def _filter_no_exists_by_downloaded_torrents(self, subscribe: Subscribe, media_info: MediaInfo,
        ...
        try:
            ...
            return self._apply_download_filter_to_no_exists(
                ...
                downloaded_episodes=list(downloaded_episodes)
            )
    ...
    def _extract_episodes_from_torrents(self, torrents: List[Context]) -> Set[int]:
        """
        从已下载的种子中提取已下载的集数
        ...
        try:
            for torrent in torrents:
                ...
                if torrent_meta.episode_list:
                    downloaded_episodes.update(torrent_meta.episode_list)
                ...
                # 如果是单集种子
                    ...
                    if torrent_meta.end_episode:
                        ...
                        # 只有开始集
                        downloaded_episodes.add(torrent_meta.begin_episode)
    ...
    def _apply_download_filter_to_no_exists(self, subscribe_name: str, no_exists: Dict, mediakey: Union[str, int],
                                            ...
                                            start_episode: Optional[int], downloaded_episodes: List[int]) -> Dict:
        """
        应用已下载集数过滤到缺失剧集信息
        ...
        :param downloaded_episodes: 已下载集数列表
        :return: 过滤后的缺失剧集信息
        """
        try:
            # 检查参数有效性
            ...
            # 从已下载集中筛选出数字类型的集数（排除"s1"这样的季标识）
            numeric_downloaded = [ep for ep in downloaded_episodes if isinstance(ep, int)]

            # 更新剧集列表：从原缺失列表中移除已下载的集数
            episodes = list(set(episode_list).difference(set(numeric_downloaded)))
