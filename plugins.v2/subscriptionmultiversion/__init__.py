...
from app.modules.searcher import SearchModule
from app.modules.downloader import DownloaderModule
...
class SubscriptionMultiVersion(_PluginBase):
    ...
    _search_module: SearchModule = None
    _downloader_module: DownloaderModule = None
    ...
    def init_plugin(self, config: dict = None):
        ...
        self._downloader_module = DownloaderModule()
    ...
    def filter_torrents(self, context: ActionContext, **kwargs) -> Tuple[bool, ActionContext]:
        ...
        try:
            ...
            for torrent in downloaded_torrents:
                if torrent.context.get("downloaded"):
                    # 检查已下载种子是否通过过滤
            ...
            for torrent in all_torrents:
                if not torrent.context.get("downloaded"):
                    # 检查是否已经被已下载种子覆盖
    ...
    def _get_downloaded_torrents_from_subscribes(self, subscribes: List[Subscribe]) -> List[Context]:
        ...
        try:
            ...
            # 从下载器获取已下载的种子
            downloaders = self._downloader_module.get_downloaders()
            ...
            for downloader in downloaders:
                try:
                    ...
                    logger.error(f"从下载器 {downloader.name} 获取种子失败: {str(e)}")
                    continue
    ...
    def _search_site_torrents(self, media_info: MediaInfo, search_sites: List[str], subscribe: Subscribe) -> List[Context]:
        ...
        try:
            ...
            # 执行搜索
            search_results = self._search_module.search(search_params)
            
            for result in search_results:
                try:
                    # 转换为Context对象
                    context_obj = Context()
                    context_obj.torrent_info = result
                    context_obj.context = {
                        "downloaded": False,
                    ...
                    site_torrents.append(context_obj)
    ...
    def _check_torrent_filter(self, torrent: Context, filter_params: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # 调用过滤模块
            return self._filter_module.filter_torrent(
    ...
    def _update_subscribe_status(self, filtered_torrents: List[Context], subscribes: List[Subscribe]):
        ...
        try:
            ...
            for torrent in filtered_torrents:
                if torrent.context.get("downloaded"):
                    # 已下载种子通过过滤，更新对应订阅状态
                    subscribe_id = torrent.context.get("subscribe_id")
                    if subscribe_id:
                        # 更新订阅状态
                        self._subscribe_oper.update_subscribe_status(
                            subscribe_id,
                            status="completed"
                        )
                        # 发送通知
                        self.post_message(
                            title="订阅完成",
                            text=f"订阅 {subscribe_id} 已完成过滤检查"
                        )
